...
func (c *Client) ScrapeWithWorkerPool(urls []string, selector string) ([]ScrapeResult, []error) {
	...
	p.done() // close jobs channel — workers will exit after draining
...
func (c *Client) RunBulkScrape(urls []string, selector string) BulkScrapeResponse {
	...
	for i, u := range urls {
		p.submit(scrapeJob{index: i, url: u, selector: selector})
	}
	p.done()
	...
	for r := range p.results {
		...
		resp.Results[r.index] = row // stable ordering via index
	}
